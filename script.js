...
function renderTaskTable() {
  ...
  tbody.querySelectorAll("input, select").forEach(el => {
    el.addEventListener("change", () => {
      ...
      if (!task) return;
      if (["arrivalTime", "burstTime", "deadline", "priority"].includes(field)) {
        task[field] = Number(el.value);
      } else {
        task[field] = el.value;
      }
      computeSchedules();
  ...
  document.getElementById("quantum-input").value = quantum;
}
...
function renderAll() {
  ...
  const isCompare = algorithm === "compare";
  const rrSection = document.getElementById("rr-section");
  const edfSection = document.getElementById("edf-section");

  if (isCompare || algorithm === "round-robin") {
    rrSection.style.display = "block";
    renderGantt(rrResult.gantt, "rr-gantt", "Round Robin Schedule");
    renderMetrics(rrResult, "rr-metrics", "Round Robin", isCompare ? edfResult : undefined);
  } else { rrSection.style.display = "none"; }

  if (isCompare || algorithm === "edf") {
    edfSection.style.display = "block";
    renderGantt(edfResult.gantt, "edf-gantt", "EDF (Earliest Deadline First) Schedule");
    renderMetrics(edfResult, "edf-metrics", "EDF", isCompare ? rrResult : undefined);
  } else { edfSection.style.display = "none"; }

  renderComparison();

  // Update algo buttons
  document.querySelectorAll(".algo-group .btn").forEach(b => {
...
function runInterval() {
  ...
  const maxTime = Math.max(...rrResult.gantt.map(g => g.endTime), ...edfResult.gantt.map(g => g.endTime), 1);
  intervalId = setInterval(() => {
    if (currentTime >= maxTime) { pauseSim(); return; }
    currentTime++;
    renderAll();
  }, 1000 / speed);
}
...
function updateControlButtons() {
  document.getElementById("btn-start").style.display = isRunning ? "none" : "inline-flex";
  document.getElementById("btn-pause").style.display = isRunning ? "inline-flex" : "none";
}

function addTask() {
  const id = "t" + Date.now();
  tasks.push({ id, name: "New Task", type: "normal", direction: "north", arrivalTime: 0, burstTime: 2, deadline: 10, priority: 3 });
  computeSchedules();
  renderAll();
}

function randomizeTasks() {
  const names = ["Lane Traffic", "Ambulance", "Fire Truck", "Bus Route", "Police Car", "VIP Escort"];
  tasks = Array.from({ length: 5 + Math.floor(Math.random() * 4) }, (_, i) => {
    const isEmg = Math.random() < 0.2;
    const isFT = !isEmg && Math.random() < 0.15;
    const burst = 1 + Math.floor(Math.random() * 6);
    const arrival = Math.floor(Math.random() * 10);
    const dir = DIRECTIONS[Math.floor(Math.random() * 4)];
    return {
      id: "t" + Date.now() + "-" + i,
      name: isEmg ? names[1] : (isFT ? names[2] : dir[0].toUpperCase() + dir.slice(1) + " " + names[0]),
      type: isEmg ? "emergency" : (isFT ? "firetruck" : "normal"),
      direction: dir,
      arrivalTime: arrival,
      burstTime: burst,
      deadline: arrival + burst + 2 + Math.floor(Math.random() * 10),
      priority: isEmg ? 1 : (isFT ? 0 : 2 + Math.floor(Math.random() * 4))
    };
  });
  // Clear color map
  Object.keys(taskColorMap).forEach(k => delete taskColorMap[k]);
  computeSchedules();
  renderAll();
}
...
document.addEventListener("DOMContentLoaded", () => {
  // Algo buttons
  document.querySelectorAll(".algo-group .btn").forEach(b => {
    b.addEventListener("click", () => { algorithm = b.dataset.algo; computeSchedules(); renderAll(); });
  });
  ...
  document.getElementById("btn-random").addEventListener("click", randomizeTasks);
